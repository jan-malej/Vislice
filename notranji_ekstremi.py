...
def ekstremi(sez):
    ...
    for i in range(1, len(sez) - 1):
        if sez[i] <= sez[i - 1] and sez[i] < sez[i + 1]:
            output.append((i, sez[i], '-'))
        if sez[i] >= sez[i - 1] and sez[i] > sez[i + 1]:
            output.append((i, sez[i], '+'))
    return output
...
def najvec_padajocih_razponov(t):
    sez = ekstremi(t)
    razponi = [abs(sez[j][1] - sez[j - 1][1]) for j in range(1, len(sez))]
    i_opt = -1
    dolzina_opt = -1
    i = 0
    while i < len(razponi):
        j = i + 1
        while j < len(razponi) and razponi[j - 1] >= razponi[j]:
            j += 1
        dolzina = j - i
        if dolzina > dolzina_opt:
            i_opt = i
            dolzina_opt = dolzina 
        i = j 
    j_opt = i_opt + dolzina_opt
    return sez[i_opt + 1][0], sez[j_opt][0]
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyNDI2NCwidXNlciI6NzM1OX0:1nh9PS:cd8FRKVbb9ezVK2wLG7XNwt5Zzg'
        try:
            Check.equal('f(z)', 5)
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyNDI2NSwidXNlciI6NzM1OX0:1nh9PS:7pHemwuRnjYcS0zn5F2u_uver3M'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyNDI2NiwidXNlciI6NzM1OX0:1nh9PS:6xzCbhW9XrHa9jXPdxDtVRXhgd4'
        try:
